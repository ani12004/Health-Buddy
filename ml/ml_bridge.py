...
MODEL_DIR = os.path.join(os.path.dirname(__file__), "..", "HealthBuddy_Models_v9")
...
def calibrate(disease: str, raw_prob: float) -> float:
    # Map disease key (snake_case) to calibration config key (title case)
    label = DISEASE_LABELS.get(disease, disease)
    cfg: Dict[str, Any] = CAL_CFG.get(label, {})
